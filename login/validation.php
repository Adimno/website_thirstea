...
if ($num == 1) {
    ...
    } else if ($row['role'] == 'cp_user') {
        // If the user is a CP (Control Panel) user
        $_SESSION['role'] = 'cp_user'; // Store the user role in session for mobile navigation
        echo json_encode(array('status' => 'success', 'message' => 'Login successful', 'role' => 'cp_user'));
